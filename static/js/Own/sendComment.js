...
$(document).ready(function () {
    $('.comment_edit').focus(function () {
        $('.comment_send').animate({width:'toggle'},350);
        $('.category_delete').mouseenter(function () {
            $('.category_delete_text').html("<span class='glyphicon glyphicon-trash'></span>");
        }).mouseleave(function () {
            $('.category_delete_text').html('Удалить ?')
        });
